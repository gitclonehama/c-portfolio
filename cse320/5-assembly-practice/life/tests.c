...
void _life(int *cells, int width, int height) 
{
	int i;int saved;int size=width*height;int*cells2=(int *)calloc(size, sizeof(int));for (i = 0; i < size*32; i++){if (_getbit(cells, size, i) == 0) continue;else {
	saved = 1;_setbit(cells2, size, i, 0);while (saved == 1){i++;if (i < size*32){saved = _getbit(cells, size, i);_setbit(cells2, size, i, 1);} else{saved = 0;}}}}
	memcpy(cells, cells2, size * sizeof(int));free(cells2);
}
